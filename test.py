...
def get_gcp_credentials():
    ...
    if "gcp_service_account" in st.secrets:
        ...
        required_fields = ['token_uri', 'client_email', 'private_key', 'project_id']
        missing = [field for field in required_fields if field not in credentials_info]
        if missing:
            raise RuntimeError(f"❌ В secrets.toml отсутствуют поля: {', '.join(missing)}. Проверьте [gcp_service_account].")
...
def check_google_connection():
    """
    Проверка подключения к Google Sheets перед началом анализа.
    Пытается открыть тестовую таблицу или просто авторизоваться.
    """
    client = authenticate_google_sheets()
    if client is None:
        st.error("❌ Подключение к Google Sheets не удалось. Проверьте credentials и доступы.")
        return False
    
    try:
        # Тест: Попытка открыть существующую таблицу (замените на вашу, если нужно)
        spreadsheet = client.open('DC_stat')
        st.success("✅ Подключение к Google Sheets успешно проверено!")
        return True
    except gspread.exceptions.SpreadsheetNotFound:
        st.warning("⚠️ Таблица 'DC_stat' не найдена, но подключение работает. Создайте таблицу или проверьте название.")
        return True  # Считаем успешным, если авторизация прошла, даже если таблицы нет
    except Exception as e:
        st.error(f"❌ Ошибка при тесте подключения: {str(e)}")
        return False
...
def main():
    ...
    with col1:
        ...
        if not files_uploaded:
            ...
            # Проверка подключения к Google перед кнопкой
            if st.button("🔌 Проверить подключение к Google"):
                check_google_connection()
            
            if st.button("🚀 Начать обработку", type="primary"):
                
                # Проверка подключения перед началом анализа
